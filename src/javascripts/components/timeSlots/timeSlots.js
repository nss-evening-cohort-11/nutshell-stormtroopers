  ...
  timeSlots.forEach((timeSlot) => {
    if (timeSlot.reservations[0] !== undefined) {
      ...
      timeSlot.reservations.forEach((reservation) => {
        ...
        domString += `<div class="d-flex justify-content-between"><span class="reserved-timeslot text-center"><strong>(Reserved Time)</strong> ${timeSlot.time}</span>`;
        domString += `<button class="edit-reservation-button" id="${reservation.id}">View Reservation</button>`;
